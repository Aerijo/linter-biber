...
module.exports = class Linter {
  constructor(editor, linterInterface, ignoredLints=[], maxLines=1e4) {
    ...
    this.style = this.styleRegistry.getStyle("apa");
  ...
  */
  ...
  parseField(fieldNode, props) {
    ...
    const fieldValue = this.getFieldValue(fieldNode);
    if (fieldValue !== null) {
      ...
      if (lintData !== null) {
        this.lintMessages.push({
          ...
          location: {
            file: this.filePath,
            position: [fieldIdentifierNode.startPosition, fieldIdentifierNode.endPosition] // NOTE: Need value node instead
          },
          excerpt: lintData.msg, // use 999 to represent missing ID
  ...
  getFieldValue(fieldNode) {
    ...
    if (valueNodeData === null) { return null; }

    let values = [];
    valueNodeData.node.namedChildren.forEach(token => {
      ...
      switch (tokenType) {
        ...
        break;
        case "identifier":
        const identifierValue = this.style.getIdentifierValue(this.getNodeValue(token));
