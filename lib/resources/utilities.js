...
function serialiseTemplate(input) {
  if (input instanceof Map) {
    let mapObj = {};
    input.forEach((value, key) => mapObj[key] = serialiseTemplate(value));
    return mapObj;
  } else if (input instanceof Set) {
    let setArray = [];
    input.forEach(value => setArray.push(serialiseTemplate(value)));
    return setArray;
  } else if (input instanceof Array) {
    return input.map(value => serialiseTemplate(value));
  } else if (input instanceof Object) {
    let keys = Object.keys(input);
    let newObject = {};
    keys.forEach(key => {
      let oldValue = input[key];
      newObject[key] = serialiseTemplate(oldValue);
    });
    return newObject;
  } else {
    return input;
...
function isEmptyTemplate(c) { // [] == false
  return !(c.constants.length > 0 ||
    c.entries.size > 0 ||
    c.fields.size > 0 ||
    c.entryFields.size > 0 ||
    c.universalFields.size > 0 ||
    c.entryConstraints.length > 0 ||
    c.universalConstraints.length > 0 ||
    c.resetPriorEntries ||
    c.resetPriorFields ||
...
function setOverlap(a1, a2) {
  // a1 has something not in a2, and a2 has something not in a1
  const areDisjoint = a1.some(str => !a2.includes(str)) && a2.some(str => !a1.includes(str));
  return areDisjoint ? 0 : a1.length < a2.length ? 1 : 2;
}


function buildProtoStyleFromTemplates(templates, baseProtoStyle=null) {
  let protoStyle = baseProtoStyle;
  if (!protoStyle) {
    protoStyle = {
      constants: new Map(),            // name      -> constant
      entries: new Map(),              // name      -> properties
      fields: new Map(),               // name      -> properties
      entryFields: new Map(),          // entryName -> Set (fieldNames)
      universalFields: new Set(),      // names
      entryConstraints: new Map(),     // entryName -> Set (constraints)
      universalConstraints: new Set(), // constraints
    };
  }
  ...
  for (let i = 0; i < templates.length; i++) {
    ...
    entryNames.forEach(name => {
      ...
      if (protoStyle.entryFields.has(name)) {
        const existingFields = protoStyle.entryFields.get(name);
        fields.forEach(field => existingFields.add(field));
      } else {
        protoStyle.entryFields.set(name, new Set(fields));
      }
    });

    template.universalFields.forEach(field => protoStyle.universalFields.add(field));
    ...
    template.entryConstraints.forEach(constraintData => {
      ...
      names.forEach(name => {
        if (protoStyle.entryConstraints.has(name)) {
          const existingConstraints = protoStyle.entryConstraints.get(name);
          constraints.forEach(constraint => existingConstraints.add(constraint));
        } else {
          protoStyle.entryConstraints.set(name, new Set(constraints));
        }
      });
    });

    template.universalConstraints.forEach(constraint => {
      protoStyle.universalConstraints.add(constraint);
    });
  }
