...
async function getStyleDef(filePath) {
  ...
  let validatedCommands = postProcessCommands(parsedCommands);
  ...
  if (command.options === null) {
    ...
    entries.forEach(entry => {
      ...
      if (entryConstraints) {
        ...
        parsedCommands.entryConstraints.set(entry, parsedConstraints);
      }
    });
  }
}
...
function parseConstraintText(text) {
  ...
  const parsedConstraints = {
    ...
    choiceRequiredXOR: new Set(),
    choiceRequiredOR: new Set(),
    conditionalFields: new Set()
  };

  constraintList.forEach(constraint => {
    switch (constraint.options.type) {
      case "mandatory":
        addMandatoryFields(constraint, parsedConstraints);
        break;
    }
  });
...
function addMandatoryFields(constraint, parsedConstraints) {
  constraint.body.forEach(prop => {
    switch (prop.name) {
      case "constraintfield":
        parsedConstraints.requiredFields.add(prop.body.trim());
        break;
      case "constraintfieldsxor":
        const xorlist = [];
        prop.body.forEach(fieldProp => xorlist.push(fieldProp.body));
        parsedConstraints.choiceRequiredXOR.add(xorlist);
        break;
      case "constraintfieldxor":
        const orlist = [];
        prop.body.forEach(fieldProp => orlist.push(fieldProp.body));
        parsedConstraints.choiceRequiredOR.add(orlist);
        break;
    }
  });
}
