...
async function main() {
  const defaultModelFile = await findFilesByExtension("./", ".dbx");

  defaultCommands = await gatherStyleCommands(defaultModelFile[0]);

  let externalStylePaths = await findFilesByExtension(packagesPath, ".dbx");
...
async function gatherStyleCommands(filePath) {
  ...
  const gatheredCommands = (defaultCommands && defaultCommands.map(i => i)) || [];
