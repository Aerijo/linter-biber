...
const { entryFields, entryAliases, ignoredEntries, unsupportedEntries, fieldAliases, specialFields } = require("./cite_styles/default.js");
...
class Linter {
  ...
  */
  async lint(event) { // jshint ignore:line
    ...
    */
    ...
    try {
      ...
      // errors are thrown when we start parsing before the previous is finished. Can happen with enormous files.
      throw error;
    }
  ...
  generateLintsForNode(node) {
    ...
    switch (node.type) {
      ...
      case "comment_command":
        ...
        range = [atSignNode.endPosition, openStringNode.startPosition];
      ...
      case "junk":
        ...
        this.makeWarning({ range, id: "001", msg: "Junk throws a warning", detail: "I recommend disabling this warning" });
        break;
      case "string_command":
    ...
    for (let i = 0; i < commentText.length; i++) {
      switch (commentText.charAt(i)) {
        ...
        case "}":
          ...
          if (braceCount < 0) { // theoretically impossible, because the parser should be handling this (though it may throw an error)
            this.makeError({ position: [row, col], id: "021", msg: "Unbalanced }", detail: "With biber, there is no way to escape characters. If you need a brace, try using a TeX command." });
          }
          break;
        case ")":
          parenCount -= 1;
          ...
          if (parenCount < 0) {
            this.makeError({ position: [row, col], id: "022", msg: "Unbalanced )", detail: "With biber, there is no way to escape characters. If you need a parenthesis, try using making the outer delims braces, or use a TeX command." });
          } else if (parenCount === 0) {
            if (braceCount > 0) {
              this.makeError({ position: [lastOpenBraceRow, lastOpenBraceCol], id: "021", msg: "Unbalanced }", detail: "Braces must still be balanced when inside a parenthesis delimited comment" });
            } else if (braceCount < 0) {
              this.makeError({ position: [lastCloseBraceRow, lastCloseBraceCol], id: "021", msg: "Unbalanced {", detail: "Braces must still be balanced when inside a parenthesis delimited comment" });
            }
          }
          break;
        case "\n":
  ...
  getInclusiveTextInNodes(startNode, endNode) {
    return this.nativeBuffer.getTextInRange({ start: startNode.startPosition, end: endNode.endPosition });
  }
  ...
  */
  ...
  makeError(details) {
    this.lintMessages.push({
      ...
      location: {
        file: this.filePath,
        position: details.range || details.position ? [details.position, [details.position[0], details.position[1] + 1]] : false || [details.node.startPosition, details.node.endPosition]
      },
