...
console.log("IS INT?", MAX_NUM_LINES % 1 === 0);
  ...
  */
  ...
  parseEntry(node) {
    ...
    if (keyName.length === 0) { // we look at the key before checking if entry is supported (because key is still recognised by biber)
      this.makeWarning({ node: nameNode, id: "012", msg: "Empty key name" });
    } else {
      if (this.keys.has(keyName)) {
        ...
        this.makeWarning({ node: keyNode, id: "011", msg: `Duplicate entry key`, detail: `(Original on line ${line}, column ${col}). Only the first entry for each key is used. Later entries sharing the key name are ignored` });
      } else {
    ...
    let crossref = false;
    node.namedChildren.forEach(fieldNode => {
      if (fieldNode.type !== "field") { return; } // comments are also possible
      ...
      if (/^(url|event|orig)?date$/.test(fieldName) && fieldNode.children.length >= 3) {
        ...
        if (!dateValue.includes("#") && !validDateFormat(dateValue)) { // silently ignore if macros and concatenation are involved (may come back to this)
          this.makeWarning({ node: dateNode, id: "010", msg: "Invalid date format" });
        }
      }
    });
